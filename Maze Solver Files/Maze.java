...
public class Maze {
    ...
    public Maze (String fileName) {
        ...
        // number of pixels
        int numOfPixels = (this.image.height() * this.image.width());
        // creates new Union Find
        this.uf = new UnionFind(numOfPixels);
        boolean visitRed = false;
        for(int y = 0; y < this.image.height(); y++) {
            for(int x = 0; x < this.image.width(); x++) {
                // finds the red dots, constructs start and end point, and color the dots to white
                if((this.image.isRed(x,y) == true) && (visitRed == false)) {
                    this.startX = x;
                    this.startY = y;
                    this.image.set(x, y, new Color(0xFF, 0xFF, 0xFF));
                    visitRed = true;
                }
                if((this.image.isRed(x,y) == true) && (visitRed == true)) {
                    this.endX = x;
                    this.endY = y;
                    this.image.set(x, y, new Color(0xFF, 0xFF, 0xFF));
                }
                // creates the sets of the image
                boolean myColor = this.image.isOn(x,y);
                // neighbor pixel - down
                if(((y + 1) < this.image.height()) && (myColor == this.image.isOn(x, y+1))) {
                    connect(x, y, x, y+1);
                }
                // neighbor pixel - right
                if(((x + 1) < this.image.width()) && myColor == this.image.isOn(x+1, y)) {
                    connect(x, y, x+1, y);
                }
            }
        }
    ...
    private int pixelToId (int x, int y) {
        // creates unique ID between 0 and width * height - 1
        int id = ((y * this.image.width()) + x + 1);
        return id;
    }
    ...
    public void connect (int x1, int y1, int x2, int y2) {
        if(this.image.isOn(x1, y1) == this.image.isOn(x2, y2)) {
            int i = pixelToId(x1, y1);
            int j = pixelToId(x2, y2);
            this.uf.union(this.uf.find(i), this.uf.find(j));
        }
    }
    ...
    public boolean areConnected (int x1, int y1, int x2, int y2) {
        ...
        if(this.uf.find(i) == this.uf.find(j)) {
            return true;
        }
        else {
            return false;
    ...
    public boolean mazeHasSolution() {
        int startID = pixelToId(startX, startY);
        int endID = pixelToId(endX, endY);
        if(this.uf.find(startID) == this.uf.find(endID)) {
            return true;
        }
        else {
            return false;
